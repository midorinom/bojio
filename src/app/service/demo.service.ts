...
export class DemoService {
    ...
    async getMessages() {
        return await lastValueFrom(this.http.get<any>('http://3.1.206.9:8000/demo/get', { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async addMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://3.1.206.9:8000/demo/add', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async updateMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://3.1.206.9:8000/demo/update', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async deleteMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://3.1.206.9:8000/demo/delete', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }
}
