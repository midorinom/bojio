...
export class DemoService {
    ...
    // Testing Deployment
    async getMessages() {
        return await lastValueFrom(this.http.get<any>('http://127.0.0.1:5000/demo/get', { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async addMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://127.0.0.1:5000/demo/add', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async updateMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://127.0.0.1:5000/demo/update', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }

    async deleteMessage(message: DemoMessage) {
        return await lastValueFrom(this.http.post<any>('http://127.0.0.1:5000/demo/delete', message, { observe: 'response' }))
            .then(response => response)
            .catch(error => error)
    }
}
